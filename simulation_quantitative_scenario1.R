...
#pseudo probabitlies
pseudo_prob <- function(alpha, prob1, prob2, prob3){
  prob_v1 = alpha*prob1 + (1-alpha)*prob2
  prob_v2 = prob2
  prob_v3 = prob3
  prob_v1 = prob_v1/sum(prob_v1)
  prob_v2 = prob_v2/sum(prob_v2)
  prob_v3 = prob_v3/sum(prob_v3)
  return(list(prob_v1, prob_v2, prob_v3))
}
...
generate_samples <- function(alpha, prob, count_data, sample_size, num_gene, otu_selected, coefs, phenotype, norm_method){
  ...
  if (phenotype == "sin"){
    for (i in 1:sample_size){
      simulation_adjust$status[i] = 1e-13 *sin(as.matrix(simulation_adjust[i,otu_selected]) %*% coefs)* exp(as.matrix(simulation_adjust[i,otu_selected]) %*% coefs) + noise[i]
    }
  }
...
prob_v = pseudo_prob(alpha, prob_data1, prob_data2, prob_data3)
prob_v1 = prob_v[[1]]
prob_v2 = prob_v[[2]]
prob_v3 = prob_v[[3]]
